# Mechanical_Vents.py

from typing import Any, Dict, List
from PySide6.QtWidgets import (
    QWidget,
    QVBoxLayout,
    QHBoxLayout,
    QPushButton,
    QScrollArea,
    QFormLayout,
    QLineEdit,
    QLabel,
    QGroupBox,
    QMessageBox,
)
from PySide6.QtCore import Qt
from namelist_definitions import NAMELISTS
from tab_base import NamelistTabBase, parse_namelist_value

class VentEntryWidget(QGroupBox):
    """Widget that captures one VENT entry for Mechanical Vents."""

    def __init__(self, defaults: Dict[str, Dict[str, Any]], remove_callback=None, preset: Dict[str, Any] = None):
        super().__init__("Mechanical Vent Definition")
        self.defaults = defaults
        self.remove_callback = remove_callback
        self.form = QFormLayout()
        self.fields: Dict[str, QLineEdit] = {}

        entry_values = preset or {}
        # Ensure the type is set to MECHANICAL for new entries
        if not preset:
            entry_values["type"] = "MECHANICAL"

        for var_name, metadata in self.defaults.items():
            line_edit = QLineEdit()
            # Get value from preset, then metadata default, then empty string
            val = entry_values.get(var_name, metadata.get("default", ""))
            
            if isinstance(val, (list, tuple)):
                val = ", ".join(str(x) for x in val)
            
            line_edit.setText(str(val))
            
            # If it's the 'type' field, we might want to keep it read-only or hidden 
            # since this tab is specifically for MECHANICAL types.
            if var_name.lower() == "type":
                line_edit.setReadOnly(True)
                line_edit.setStyleSheet("background-color: #f0f0f0;")

            self.form.addRow(f"{var_name}:", line_edit)
            self.fields[var_name] = line_edit

        self.delete_button = QPushButton("Remove Vent")
        self.delete_button.clicked.connect(self._trigger_removal)

        bottom_layout = QHBoxLayout()
        bottom_layout.addStretch()
        bottom_layout.addWidget(self.delete_button)

        wrapper = QVBoxLayout()
        wrapper.addLayout(self.form)
        wrapper.addLayout(bottom_layout)
        self.setLayout(wrapper)

    def _trigger_removal(self):
        if self.remove_callback:
            self.remove_callback(self)

    def get_values(self) -> Dict[str, Any]:
        values = {}
        for var_name, widget in self.fields.items():
            meta = self.defaults[var_name]
            values[var_name] = parse_namelist_value(widget.text(), meta)
        # Force type to MECHANICAL just in case
        values["type"] = "MECHANICAL"
        return values

class Mechanical_Vents(QWidget):
    """Tab that allows adding, editing, and removing multiple MECHANICAL VENT namelist entries."""

    def __init__(self, parent=None):
        super().__init__(parent)
        outer_layout = QVBoxLayout(self)

        header = QLabel("Mechanical Vents (VENT)")
        header.setAlignment(Qt.AlignCenter)
        header.setStyleSheet("font-weight: bold; font-size: 14pt;")
        outer_layout.addWidget(header)

        self.scroll = QScrollArea()
        self.scroll.setWidgetResizable(True)
        container = QWidget()
        self.entries_layout = QVBoxLayout(container)
        self.entries_layout.addStretch()
        self.scroll.setWidget(container)
        outer_layout.addWidget(self.scroll)

        button_layout = QHBoxLayout()
        self.add_button = QPushButton("Add Mechanical Vent")
        self.add_button.clicked.connect(self._add_empty_entry)
        button_layout.addStretch()
        button_layout.addWidget(self.add_button)
        outer_layout.addLayout(button_layout)

        self.entry_widgets: List[VentEntryWidget] = []
        # We start with no entries by default, or one if preferred. 
        # Thermal_Properties starts with one.
        self._add_empty_entry()

    def _add_empty_entry(self):
        self._add_entry()

    def _add_entry(self, preset: Dict[str, Any] = None):
        """Create a new vent entry widget and keep track of it."""
        entry_widget = VentEntryWidget(
            defaults=NAMELISTS["VENT"],
            preset=preset,
            remove_callback=self._remove_entry,
        )
        self.entry_widgets.append(entry_widget)
        # Insert before the stretch spacer
        self.entries_layout.insertWidget(self.entries_layout.count() - 1, entry_widget)

    def _remove_entry(self, widget: VentEntryWidget, *, force: bool = False):
        if not force and len(self.entry_widgets) == 1:
            QMessageBox.information(self, "Cannot remove", "At least one VENT entry is required.")
            return
        self.entry_widgets.remove(widget)
        widget.setParent(None)
        widget.deleteLater()

    def _clear_entries(self):
        for widget in list(self.entry_widgets):
            self._remove_entry(widget, force=True)

    def get_data(self) -> Dict[str, List[Dict[str, Any]]]:
        """Returns only the mechanical vents managed by this tab."""
        return {"VENT": [entry.get_values() for entry in self.entry_widgets]}

    def set_data(self, data: Dict[str, Any]):
        """Filters incoming namelist data for VENT records where TYPE='MECHANICAL'."""
        all_vents = data.get("VENT", [])
        
        # f90nml might return a single dict if there's only one VENT record
        if isinstance(all_vents, dict):
            all_vents = [all_vents]
        
        # Filter for MECHANICAL type (case-insensitive)
        mech_vents = [
            v for v in all_vents 
            if str(v.get("type", "")).upper() == "MECHANICAL"
        ]
        
        self._clear_entries()
        if not mech_vents:
            self._add_empty_entry()
        else:
            for entry in mech_vents:
                self._add_entry(preset=entry)
