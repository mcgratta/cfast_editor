# Ceiling_Vents.py

from typing import Any, Dict, List

from PySide6.QtWidgets import (
    QWidget,
    QVBoxLayout,
    QHBoxLayout,
    QPushButton,
    QScrollArea,
    QLineEdit,
    QLabel,
    QGroupBox,
    QMessageBox,
    QFormLayout,
)
from PySide6.QtCore import Qt

from namelist_definitions import NAMELISTS
from tab_base import parse_namelist_value

ALLOWED_VENT_TYPES = {"CEILING", "FLOOR"}


def _is_ceiling_or_floor(entry: Dict[str, Any]) -> bool:
    """Return True when the entry explicitly targets ceiling/floor vents."""
    raw_type = entry.get("type")
    if raw_type is None:
        return False
    return str(raw_type).strip().upper() in ALLOWED_VENT_TYPES


class VentEntryWidget(QGroupBox):
    """Widget that captures the fields for one VENT record."""

    def __init__(
        self,
        defaults: Dict[str, Dict[str, Any]],
        remove_callback=None,
        preset: Dict[str, Any] = None,
    ):
        super().__init__("Ceiling/Floor Vent")
        self.defaults = defaults
        self.remove_callback = remove_callback
        self.form = QFormLayout()
        self.fields: Dict[str, QLineEdit] = {}

        entry_values = preset or {}
        for var_name, metadata in self.defaults.items():
            line_edit = QLineEdit()
            value = entry_values.get(var_name, metadata.get("default", ""))
            if isinstance(value, (list, tuple)):
                value = ", ".join(str(v) for v in value)
            line_edit.setText(str(value))
            self.form.addRow(f"{var_name}:", line_edit)
            self.fields[var_name] = line_edit

        self.delete_button = QPushButton("Remove Vent")
        self.delete_button.clicked.connect(self._trigger_removal)
        bottom_layout = QHBoxLayout()
        bottom_layout.addStretch()
        bottom_layout.addWidget(self.delete_button)

        outer_layout = QVBoxLayout()
        outer_layout.addLayout(self.form)
        outer_layout.addLayout(bottom_layout)
        self.setLayout(outer_layout)

    def _trigger_removal(self):
        if self.remove_callback:
            self.remove_callback(self)

    def get_values(self) -> Dict[str, Any]:
        """Return the parsed values for this vent entry."""
        values = {}
        for var_name, widget in self.fields.items():
            metadata = self.defaults[var_name]
            values[var_name] = parse_namelist_value(widget.text(), metadata)
        return values

    def set_values(self, values: Dict[str, Any]):
        """Overwrite the widgets with a provided dataset."""
        for var_name, widget in self.fields.items():
            incoming = values.get(var_name, self.defaults[var_name].get("default", ""))
            if isinstance(incoming, (list, tuple)):
                incoming = ", ".join(str(v) for v in incoming)
            widget.setText(str(incoming))


class Ceiling_Vents(QWidget):
    """Manages a list of ceiling/floor VENT records."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.defaults = NAMELISTS["VENT"]

        outer_layout = QVBoxLayout(self)

        header = QLabel("Ceiling/Floor Vents (VENT)")
        header.setAlignment(Qt.AlignCenter)
        header.setStyleSheet("font-weight: bold; font-size: 14pt;")
        outer_layout.addWidget(header)

        self.scroll = QScrollArea()
        self.scroll.setWidgetResizable(True)
        container = QWidget()
        self.entries_layout = QVBoxLayout(container)
        self.entries_layout.addStretch()
        self.scroll.setWidget(container)
        outer_layout.addWidget(self.scroll)

        button_layout = QHBoxLayout()
        self.add_button = QPushButton("Add Vent Entry")
        self.add_button.clicked.connect(self._add_empty_entry)
        button_layout.addStretch()
        button_layout.addWidget(self.add_button)
        outer_layout.addLayout(button_layout)

        self.entry_widgets: List[VentEntryWidget] = []
        self._add_empty_entry()

    def _add_empty_entry(self, preset: Dict[str, Any] = None):
        entry_widget = VentEntryWidget(
            defaults=self.defaults,
            preset=preset or {"type": "CEILING"},
            remove_callback=self._remove_entry,
        )
        self.entry_widgets.append(entry_widget)
        self.entries_layout.insertWidget(self.entries_layout.count() - 1, entry_widget)

    def _remove_entry(self, widget: VentEntryWidget, *, force: bool = False):
        if not force and len(self.entry_widgets) == 1:
            QMessageBox.information(self, "Cannot remove", "At least one vent entry is required.")
            return
        if widget in self.entry_widgets:
            self.entry_widgets.remove(widget)
            widget.setParent(None)
            widget.deleteLater()

    def _clear_entries(self):
        for widget in list(self.entry_widgets):
            self._remove_entry(widget, force=True)

    def get_data(self) -> Dict[str, List[Dict[str, Any]]]:
        vents = [entry.get_values() for entry in self.entry_widgets]
        vents = [vent for vent in vents if _is_ceiling_or_floor(vent)]
        return {"VENT": vents}

    def set_data(self, data: Dict[str, Any]):
        vents = data.get("VENT", [])
        if isinstance(vents, dict):
            vents = [vents]
        vents = [vent for vent in vents if _is_ceiling_or_floor(vent)]
        vents = vents or [{"type": "CEILING"}]
        self._clear_entries()
        for entry in vents:
            self._add_empty_entry(preset=entry)
